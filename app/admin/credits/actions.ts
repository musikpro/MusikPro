...
export async function setCredits(_previous: AdminActionState, formData: FormData): Promise<AdminActionState> {
  try {
    ...
    return { ok: true, message: "Solde de crédits mis à jour." };
  } catch (error) {
    return { ok: false, message: actionErrorMessage(error, "Impossible de mettre à jour ce solde de crédits.") };
  }
}
