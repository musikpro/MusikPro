...
export default async function ChariowProviderPage() {
  ...
  return <AdminPage>
    ...
    <AdminPageHeader eyebrow="Paiements" title="Chariow" description="Mobile Money et carte bancaire via une page de paiement sécurisée." />
      ...
      <form action={saveChariowProvider} className="admin-stack-form admin-chariow-form">
        <div className="admin-editor-grid">
          <div className="admin-editor-field admin-editor-field-wide">
            <span>Clé API Chariow <InfoTip text="Copie la clé API privée depuis ton espace Chariow. Elle est chiffrée avant stockage et ne sera jamais réaffichée. Le champ reste verrouillé une fois configuré; clique sur « Modifier » pour la remplacer." /></span>
            <AdminSecretField name="apiKey" configured={Boolean(apiLast4)} placeholder={apiLast4 ? `Clé enregistrée ••••${apiLast4} — laisser vide pour conserver` : "Saisir la clé API Chariow"} />
          </div>
          <div className="admin-editor-field admin-editor-field-wide">
            <span>Secret du webhook <InfoTip text="Secret placé dans l’URL Pulse pour authentifier Chariow. Laisse vide pour le générer ou conserver l’existant. Le champ reste verrouillé une fois configuré; clique sur « Modifier » pour le remplacer." /></span>
            <AdminSecretField name="webhookSecret" configured={Boolean(webhookLast4)} placeholder={webhookLast4 ? `Secret enregistré ••••${webhookLast4} — laisser vide pour conserver` : "Laisser vide pour générer automatiquement"} />
          </div>
      ...
      <form action={savePlanMapping} className="admin-product-create-form">
        ...
        {chariowProductOptions.length ? (
          <div className="admin-editor-field">
            <span>Produit Chariow <InfoTip text="Détecté automatiquement depuis ton compte Chariow (clé API enregistrée ci-dessus). Recharge la page après avoir créé un nouveau produit côté Chariow." /></span>
            <AdminSelect
          ...
          <label className="admin-editor-field"><span>Identifiant du produit <InfoTip text={chariowProductsError ? "La détection automatique a échoué (clé API invalide ou Chariow indisponible). Saisis l’identifiant exact copié depuis ton espace Chariow." : "Identifiant exact du produit copié depuis ton espace Chariow. La détection automatique nécessite d’enregistrer d’abord la clé API ci-dessus."} /></span><input name="externalProductId" placeholder="prod_..." required /></label>
        )}
      ...
      <div className="admin-mapping-list">{chariowMappings.length ? chariowMappings.map((mapping) => {
        ...
        return <form action={savePlanMapping} key={mapping.id} className="admin-product-row">
          ...
          <div className="admin-product-actions">
            <button type="submit" className="admin-secondary-action" aria-label={`Enregistrer ${metadata.productName || plan?.name || "le produit"}`}><Icon i="save" size={15} />Modifier</button>
            <button type="submit" formAction={deletePlanMapping} className="admin-secondary-action is-danger" aria-label={`Supprimer ${metadata.productName || plan?.name || "le produit"}`}><Icon i="trash-2" size={15} />Supprimer</button>
          </div>
        </form>;
