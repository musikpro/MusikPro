...
import { MusicJobOwnershipError, requestMp4Conversion } from "@/lib/ai/music-jobs";
...
export async function POST(request: Request, ctx: Ctx) {
  ...
  if (!provider.allowMp4Conversion) return NextResponse.json({ error: "La conversion MP4 est désactivée." }, { status: 403 });

  const limit = await rateLimit(`music:mp4:${session.user.id}:${clientIp(request)}`, 10);
  ...
  try {
    const job = await requestMp4Conversion(parsedId.data, session.user.id);
    await writeAuditLog({ action: "musicful.mp4.requested", actorId: session.user.id, targetType: "music_generation_job", targetId: job.id });
    return NextResponse.json({ jobId: job.id, mp4Url: job.mp4Url });
  } catch (error) {
