    ...
    <div className="bg-surface flex flex-col">
      ...
      <div className="px-4 pt-3 pb-1">
        <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-primary bg-secondary px-3 py-1.5 rounded-lg">
          {demoOccasionEmoji(demo.choices.occasion)} {demo.choices.occasion} • 🎵 {demo.choices.genre}
        </span>
      </div>

      {/* Title */}
      <div className="px-4 pt-4 pb-5">
        <h1 className="font-headings font-bold text-2xl text-foreground mb-1">{t("Prêt à générer")}</h1>
        <p className="text-sm text-muted-foreground">{t("Voici un résumé de ta chanson")}</p>
      </div>
      ...
      <div className="flex-1 px-4 pb-4 overflow-y-auto space-y-6">
        ...
        {/* Lyrics Preview */}
        <div>
          <label className="text-xs font-bold text-muted-foreground uppercase">{t("Aperçu des paroles")}</label>
          <div className="bg-card border border-border rounded-lg p-3 mt-2">
            <p className="text-xs leading-relaxed text-foreground line-clamp-4">{demo.fields.lyrics.slice(0, 180)}…</p>
          </div>
          <button
            type="button"
