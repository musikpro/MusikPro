    ...
    <Preview>
      <div className="banani-screen demo-route-skeleton" aria-busy="true" aria-label="Chargement de la page">
        ...
        <div className="p-4">
          ...
          <Skeleton width="80%" height="16px" style={{ marginTop: 8, marginBottom: 24 }} />
          {variant === "profile" && (
            <Skeleton width="80px" height="80px" style={{ borderRadius: "50%", margin: "16px auto" }} />
          )}
          <div
            className={
              variant === "creation" || variant === "library" ? "grid grid-cols-2 gap-3" : "flex flex-col gap-3"
            }
          >
            {Array.from(
              {
                length: variant === "creation" ? 8 : variant === "library" ? 6 : 3,
              },
              (_, i) => (
                <Skeleton
                  key={i}
