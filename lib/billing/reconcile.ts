...
import { paymentFulfillments, payments, plans, subscriptions, credits, coupons } from "@/db/schema";
...
function rawReference(input: unknown): string | undefined {
  ...
  const meta = data?.custom_metadata ?? data?.metadata ?? data?.custom_data ?? data?.meta ?? {};
  return meta?.app_reference ?? data?.tx_ref ?? data?.reference ?? data?.merchant_reference ?? data?.ref_command;
}
...
function periodEndExpression(interval: string, succeededAt: Date) {
  ...
  return interval === "year" ? sql`${base} + interval '1 year'` : sql`${base} + interval '1 month'`;
}
...
export async function reconcilePayment(paymentId: string) {
  ...
  if (remoteAmount == null) throw new Error("Provider verification did not return amount");
  if (!remoteCurrency) throw new Error("Provider verification did not return currency");
  if (!sameMoney(remoteAmount, expectedAmount, expectedCurrency)) throw new Error("Payment amount mismatch");
  if (remoteCurrency !== expectedCurrency) throw new Error("Payment currency mismatch");
  if (reference && reference !== payment.reference) throw new Error("Payment reference mismatch");

  if (verified.status === "paid") {
    const succeededAt = paidDate(verified.raw, payment.createdAt);
    await db.update(payments).set({ status: "paid", paidAt: succeededAt }).where(eq(payments.id, payment.id));
    ...
    if (payment.userId && payment.planId) {
      ...
      if (creditFeatures.success) {
        ...
        if (!["month", "year"].includes(plan.interval)) throw new Error("Unsupported billing interval");
        await db
          ...
          .values({
            ...
            status: "active",
            renewalMode: "manual",
            currentPeriodEnd: addCalendarPeriod(succeededAt, plan.interval as "month" | "year"),
        ...
        await db
          ...
          .values({ id: randomUUID(), paymentId: payment.id, provider: provider.id, appliedAt: new Date() })
