import { bictorysPayloadSchema } from "@/lib/validation/payment-providers";
...
export class BictorysProvider extends HttpPaymentProvider {
  protected async json(url: string, init: RequestInit = {}) {
    return bictorysPayloadSchema.parse(await super.json(url, init));
  }
  id = "bictorys" as const;
  async createCheckout(input: CheckoutInput): Promise<CheckoutResult> {
    const response = await this.json(`${base()}/pay/v1/charges`, {
      method: "POST",
      headers: headers(),
      cache: "no-store",
      body: JSON.stringify({
  ...
  async verifyWebhook(request: Request) {
    const secret = requireEnv("BICTORYS_WEBHOOK_SECRET");
    ...
    const sent = request.headers.get("x-secret-key");
    return !!sent && safe(sent, secret);
  }
  async parseWebhook(request: Request) {
    const p = bictorysPayloadSchema.parse(JSON.parse(await request.text()));
    const id = String(p.transactionId || p.chargeId || p.data?.id || "");
    if (!id) throw new Error("Bictorys webhook missing transaction id");
    return {
      id: `bictorys:${id}:${p.event || p.status || "unknown"}`,
      type: String(p.event || p.status || "unknown"),
      payload: { ...p, transaction: { id }, reference: p.paymentReference },
    };
  }
}
