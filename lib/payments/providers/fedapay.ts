import { fedapayPayloadSchema } from "@/lib/validation/payment-providers";
...
function statusOf(value?: string): CheckoutResult["status"] {
  ...
  if (["canceled", "declined", "refunded"].includes(value ?? ""))
    return "failed";
  return "pending";
}
...
export class FedapayProvider extends HttpPaymentProvider {
  ...
  async verifyPayment(externalId: string): Promise<CheckoutResult> {
    if (!/^\d+$/.test(externalId))
      throw new Error("Invalid FedaPay transaction id");
    const transaction = (await this.json(
      `${baseUrl()}/transactions/${externalId}`,
      {
        headers: this.headers(),
        cache: "no-store",
      },
    ...
    const currency = String(
      transaction.currency?.iso ??
        ...
        transaction.custom_metadata?.app_currency ??
        "",
    );
  ...
  private async constructEvent(request: Request) {
    ...
    return fedapayPayloadSchema.parse(
      sdk.Webhook.constructEvent(raw, signature, secret),
    );
  }

  async verifyWebhook(request: Request): Promise<boolean> {
    try {
      await this.constructEvent(request);
      return true;
    } catch {
      return false;
    }
  ...
  async parseWebhook(request: Request) {
    ...
    const id = String(
      event.id ??
        `${event.name ?? event.type}:${entity.id ?? entity.reference ?? "unknown"}`,
    );
