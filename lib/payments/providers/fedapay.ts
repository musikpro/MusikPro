...
const baseUrl = () =>
  process.env.FEDAPAY_ENVIRONMENT === "live" ? "https://api.fedapay.com/v1" : "https://sandbox-api.fedapay.com/v1";
...
export class FedapayProvider extends HttpPaymentProvider {
  ...
  async verifyPayment(externalId: string): Promise<CheckoutResult> {
    ...
    const currency = String(
      transaction.currency?.iso ?? transaction.currency_iso ?? transaction.custom_metadata?.app_currency ?? "",
    );
  ...
  private async constructEvent(request: Request) {
    ...
    return fedapayPayloadSchema.parse(sdk.Webhook.constructEvent(raw, signature, secret));
  }
