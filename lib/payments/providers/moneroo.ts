...
function normalizeStatus(value: unknown): CheckoutResult["status"] {
  ...
  if (["success", "successful", "completed", "paid"].some((x) => s.includes(x)))
    return "paid";
  if (
    ["failed", "cancelled", "canceled", "refunded"].some((x) => s.includes(x))
  )
    return "failed";
  return "pending";
}
...
export class MonerooProvider extends HttpPaymentProvider {
  ...
  async createCheckout(input: CheckoutInput): Promise<CheckoutResult> {
    if (!input.customer.email)
      throw new Error("Moneroo requires customer email");
    const names = (
      input.customer.name ||
      input.customer.email.split("@")[0] ||
      "Customer"
    )
      .trim()
      .split(/\s+/);
    ...
    if (!id || !checkoutUrl)
      throw new Error("Moneroo returned an incomplete checkout response");
    return {
      provider: this.id,
      externalId: id,
      status: "pending",
  ...
  async verifyPayment(externalId: string): Promise<CheckoutResult> {
    const body = await this.json(
      `${BASE}/payments/${encodeURIComponent(externalId)}/verify`,
  ...
  async verifyWebhook(request: Request) {
    ...
    const expected = createHmac("sha256", requireEnv("MONEROO_WEBHOOK_SECRET"))
      .update(raw)
      .digest("hex");
  ...
  async parseWebhook(request: Request) {
    const payload = monerooPayloadSchema.parse(
