...
const baseUrl = () => process.env.PAYDUNYA_ENVIRONMENT === "live"
...
export class PaydunyaProvider extends HttpPaymentProvider {
  ...
  async createCheckout(input: CheckoutInput): Promise<CheckoutResult> {
    if (input.money.currency !== "XOF") throw new Error("PayDunya starter route is configured for XOF; validate other currencies with your merchant account before enabling them.");
    ...
    if (String(body.response_code) !== "00" || !body.token || !body.response_text) throw new Error(`PayDunya checkout rejected: ${body.description || body.response_text || "unknown"}`);
    return { provider: this.id, externalId: String(body.token), status: "pending", checkoutUrl: String(body.response_text), raw: body };
  }
  ...
  async verifyPayment(externalId: string): Promise<CheckoutResult> {
    ...
    return { provider: this.id, externalId, status: statusOf(body.status), money: Number.isFinite(normalized.amount) && normalized.currency ? { amount: normalized.amount, currency: String(normalized.currency) } : undefined, raw: normalized };
  }

  private async payload(request: Request) {
    const contentType = request.headers.get("content-type") || "";
    if (contentType.includes("application/json")) return await request.json() as any;
    const form = await request.formData();
    const obj: Record<string, any> = {};
    for (const [key, value] of form.entries()) obj[key] = value;
    if (typeof obj.data === "string") {
      try { obj.data = JSON.parse(obj.data); } catch { /* keep raw */ }
    }
    return obj;
  }
  ...
  async parseWebhook(request: Request) {
    ...
    return { id: `paydunya:${token}:${status}`, type: `invoice.${status.toLowerCase()}`, payload: { ...payload, transaction: { id: token } } };
  }
}
