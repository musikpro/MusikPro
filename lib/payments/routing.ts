...
function listMatches(values: unknown, value?: string) {
  ...
  return values.map(String).map(v => v.toUpperCase()).includes(value.toUpperCase());
}
...
export async function availableProviders(country?: string, method?: string, currency?: string): Promise<PaymentProviderId[]> {
  ...
  const rows = await db.select().from(paymentCountryRoutes)
...
export async function rankProviders(country?: string, method?: string, currency?: string): Promise<RankedProvider[]> {
  ...
  const cc = country?.toUpperCase();
  const routeRows = cc ? await db.select().from(paymentCountryRoutes)
  ...
  const attempts = await db.select().from(paymentAttempts)
    ...
    .orderBy(desc(paymentAttempts.createdAt)).limit(500);

  return providers.map((provider, index) => {
    const recent = attempts.filter(a => a.provider === provider && (!cc || a.country === cc) && (!method || a.method === method) && ["checkout_created", "provider_error"].includes(a.outcome));
    ...
    return { provider, priority, successRate, recentAttempts: recent.length, degraded, degradedWindowAttempts: recentWindow.length, degradedWindowSuccessRate, score: priority + healthPenalty + lowSamplePenalty + degradedPenalty };
  }).sort((a, b) => a.score - b.score);
}

export async function resolveProvider(country: string | undefined, requested?: PaymentProviderId, method?: string, currency?: string) {
  ...
  if (requested) {
    ...
    throw new Error(`Provider ${requested} is not enabled for ${country || "this checkout"}${method ? ` / ${method}` : ""}`);
  }
