...
async function checkNeon(): Promise<KitCheck> {
  ...
  try {
    ...
    return {
      ...
      label: "Neon PostgreSQL",
      status: "missing",
      detail: "DATABASE_URL présente mais connexion impossible.",
      group: "Services",
    };
  }
}
  ...
  try {
    ...
    return publicUrl
      ...
      : {
          ...
          label: "ngrok",
          status: "missing",
          detail: "ngrok tourne mais aucun tunnel HTTPS n'est détecté.",
          group: "Paiements",
        };
  } catch {
    return {
      ...
      label: "ngrok local",
      status: "missing",
      detail: "Non lancé. Utilise npm run payments:ngrok pour tester les webhooks localement.",
      group: "Paiements",
    };
  }
}
...
export function getMobileAppReadiness(): MobileAppReadiness {
  ...
  const mobile =
    config && typeof config === "object"
      ? (config as KitConfig & { mobileAppEnabled?: boolean; mobileApp?: Record<string, unknown> }).mobileApp || {}
      : {};
  const enabled = Boolean(
    (config as (KitConfig & { mobileAppEnabled?: boolean }) | null)?.mobileAppEnabled || mobile.enabled === true,
  );
  const platforms = Array.isArray(mobile.platforms)
    ? mobile.platforms.map(String).filter((p) => p === "android" || p === "ios")
    : ["android", "ios"];
    ...
    services: [
      {
        ...
        detail: hasCore ? "Installés." : "Installés uniquement après activation avec mobile:app:install.",
      },
      {
        label: "Capacitor Android",
        status: !platforms.includes("android") ? "warning" : androidPkg ? "ok" : "warning",
        detail: platforms.includes("android")
          ? androidPkg
            ? "Package Android installé."
            : "À installer si Android est activé."
          : "Plateforme Android non sélectionnée.",
      },
      {
        label: "Capacitor iOS",
        status: !platforms.includes("ios") ? "warning" : iosPkg ? "ok" : "warning",
        detail: platforms.includes("ios")
          ? iosPkg
            ? "Package iOS installé."
            : "À installer si iPhone est activé."
          : "Plateforme iOS non sélectionnée.",
      },
      {
        label: "Android Studio + SDK/JDK",
        status: androidStudio ? "ok" : "warning",
        detail: androidStudio
          ? "Android Studio détecté sur ce Mac."
          : "À installer/vérifier sur la machine de build Android.",
      },
      {
        label: "Xcode (macOS)",
        status: xcode ? "ok" : "warning",
        detail: xcode ? "Xcode détecté sur ce Mac." : "À installer/vérifier sur un Mac pour compiler iOS.",
      },
    ],
    steps,
    assets: [
      "Logo source haute définition",
      "Icône carrée de l’app",
      "Splash / écran de lancement",
      "Captures Android",
      "Captures iPhone",
      "Visuel promotionnel store si requis",
    ],
  };
}
...
export async function getKitDashboardChecks(): Promise<KitCheck[]> {
  ...
  checks.push({
    id: "node",
    label: "Node.js",
    status: nodeOk ? "ok" : "missing",
    detail: `Version active : ${process.versions.node} (>=20.9.0 requis).`,
    group: "Base",
  });
  checks.push({
    id: "lockfile",
    label: "package-lock.json",
    status: exists("package-lock.json") ? "ok" : "missing",
    detail: exists("package-lock.json")
      ? "Lockfile présent."
      : "Exécute npm install puis conserve package-lock.json dans Git.",
    group: "Base",
  });
  checks.push({
    id: "config",
    label: "Configuration du kit",
    status: config ? "ok" : "missing",
    detail: config
      ? "africa-saas.config.json présent."
  ...
  checks.push({
    ...
    status: claudeInstalled && claudeProjectReady ? "ok" : claudeProjectReady ? "warning" : "missing",
    detail:
      claudeInstalled && claudeProjectReady
        ? "Claude Code détecté et fichiers projet prêts."
        : claudeProjectReady
          ? "Fichiers Claude Code prêts; CLI claude non détecté dans ce shell."
          : "Exécute npm run claude-code:prepare puis npm run claude-code:check.",
    group: "Base",
  });
  checks.push({
    id: "computer-use-claude",
    label: "Computer Use — Claude Code / Anthropic",
    status: claudeComputerUseVerified ? "ok" : claudeInstalled ? "warning" : "missing",
    detail: claudeComputerUseVerified
      ? "Voyant vert : Computer Use/Browser Claude Code vérifié par un test réel."
      : claudeInstalled
        ? "Claude Code détecté, mais Computer Use/Browser n’est pas encore vérifié. Lance npm run computer-use:claude:check."
        : "Claude Code non détecté et Computer Use non vérifié. Installe/ouvre Claude Code, puis lance npm run computer-use:claude:check.",
    group: "Base",
  });
  const ownerAdminLayoutText = readText("app/admin/layout.tsx") + "\n" + readText("components/admin/AdminShell.tsx");
  const ownerProductionStateReady =
    exists("app/admin/production-doctor/page.tsx") &&
    /href[:=]\s*["']\/admin\/production-doctor["']/.test(ownerAdminLayoutText) &&
    ownerAdminLayoutText.includes("État production");
  checks.push({
  ...
  checks.push({
    id: "auth",
    label: "Better Auth",
    status: hasEnv("BETTER_AUTH_SECRET") ? "ok" : "missing",
    detail: hasEnv("BETTER_AUTH_SECRET") ? "Secret Better Auth configuré." : "BETTER_AUTH_SECRET manquant.",
    group: "Services",
  });
  ...
  checks.push({
    ...
    status: emailPasswordMode || (hasEnv("GOOGLE_CLIENT_ID") && hasEnv("GOOGLE_CLIENT_SECRET")) ? "ok" : "missing",
    detail: emailPasswordMode
      ? "Email/mot de passe activé (Resend requis en production)."
      : "Email/mot de passe désactivé : Google OAuth doit être configuré.",
    group: "Services",
  });
  ...
  const resendOk =
    hasEnv("RESEND_API_KEY") && hasEnv("EMAIL_FROM") && !/@example\.(com|org|net)$/i.test(emailFromValue);
  checks.push({
    id: "resend",
    label: "Resend",
    status: resendOk ? "ok" : resendRequired ? "missing" : "warning",
    detail: resendOk
      ? "Clé API + adresse EMAIL_FROM présentes; l’envoi réel reste à tester."
      : resendRequired
        ? "Email/mot de passe est actif : RESEND_API_KEY + EMAIL_FROM sont requis avant production."
        : "Optionnel — email/mot de passe désactivé.",
    group: "Services",
  });
  ...
  checks.push({
    id: "google",
    label: "Google OAuth",
    status: googleOk ? "ok" : googleRequired ? "missing" : "warning",
    detail: googleOk
      ? "Client ID + secret configurés."
      : googleRequired
        ? "Google OAuth activé mais identifiants incomplets."
        : "Non configuré — optionnel tant qu’un autre mode d’authentification valide est actif.",
    group: "Services",
  });

  const searchRequired = Boolean(config?.searchConsole);
  const searchConsoleDnsVerified =
    searchRequired && !hasEnv("GOOGLE_SITE_VERIFICATION")
  ...
  checks.push({
    id: "cloudinary",
    label: "Cloudinary images (optionnel)",
    status: cloudinaryOk ? "ok" : "warning",
    detail: cloudinaryOk
      ? "Variables Cloudinary présentes; un upload réel reste à tester."
      : "Non configuré. Valide pour un SaaS sans upload d’images; décision en Phase 18.",
    group: "Services",
  });
  ...
  if (!enabledProviders.length) {
    checks.push({
      id: "payments-config",
      label: "Paiements (optionnel)",
      status: "warning",
      detail:
        "Aucun provider activé. C’est valide : les paiements ne sont configurés qu’en Phase 16 si le SaaS en a besoin.",
      group: "Paiements",
    });
  }
  ...
  if (enabledProviders.length) {
    checks.push({
      id: "webhook-base",
      label: "URL webhooks",
      status: hasEnv("PAYMENT_WEBHOOK_BASE_URL") ? "ok" : "missing",
      detail: hasEnv("PAYMENT_WEBHOOK_BASE_URL")
        ? `PAYMENT_WEBHOOK_BASE_URL configurée.`
        : "Manquante. En local, utilise ngrok + npm run payments:local:apply.",
      group: "Paiements",
    });
  ...
  checks.push({
    ...
    detail:
      cspPresent && !cspHasUnsafeInline && cspHasNonce
  ...
  const turnstileEnv =
    hasEnv("TURNSTILE_SECRET_KEY") && (hasEnv("NEXT_PUBLIC_TURNSTILE_SITE_KEY") || hasEnv("TURNSTILE_SITE_KEY"));
  const turnstileClientText =
    readText("components/auth-form.tsx") + "\n" + readText("components/forgot-password-form.tsx");
  ...
  checks.push({
    ...
    detail:
      turnstileFiles && turnstileEnv && publicFormProtected
  ...
  checks.push({
    id: "health",
    label: "API health / readiness",
    status: exists("app/api/health/route.ts") && exists("app/api/readyz/route.ts") ? "ok" : "missing",
    detail: "Sondes /api/health et /api/readyz présentes.",
    group: "Qualité",
  });
  checks.push({
    id: "runtime",
    label: "Runtime API Node.js",
    status: exists("scripts/runtime-check.mjs") ? "ok" : "missing",
    detail: "Gate runtime:check présent pour empêcher un passage accidentel en Edge.",
    group: "Qualité",
  });
  checks.push({
    id: "features",
    label: "Inventaire anti-doublons",
    status: exists("config/features.json") && exists("scripts/feature-inventory.mjs") ? "ok" : "missing",
    detail: "Manifeste des responsabilités de features/routes présent.",
    group: "Qualité",
  });
  checks.push({
    id: "observability",
    label: "Observabilité de base",
    status: exists("lib/observability/logger.ts") && exists("lib/observability/request-id.ts") ? "ok" : "missing",
    detail: "Logs structurés + redaction + request IDs présents.",
    group: "Qualité",
  });
  checks.push({
    id: "tests",
    label: "Tests unitaires",
    status: exists("vitest.config.ts") && exists("tests/payments/provider-base.test.ts") ? "ok" : "missing",
    detail: "Vitest et tests des garde-fous sensibles présents.",
    group: "Qualité",
  });
  checks.push({
    id: "lint-format",
    label: "Lint / format",
    status: exists("eslint.config.mjs") && exists(".prettierrc.json") ? "ok" : "missing",
    detail: "ESLint Next.js + Prettier configurés.",
    group: "Qualité",
  });
  checks.push({
    id: "security",
    label: "Sécurité",
    status: exists("SECURITY.md") && exists("scripts/security-audit.sh") ? "ok" : "missing",
    detail: "Audit et contrôles de sécurité du kit.",
    group: "Qualité",
  });
  checks.push({
    id: "mobile",
  ...
  checks.push({
    ...
    status: exists("components/ui/skeleton.tsx") && exists("scripts/loading-check.mjs") ? "ok" : "missing",
    detail: "Primitives et contrôle de chargement présents.",
    group: "Qualité",
  });
  checks.push({
    id: "seo",
    ...
    status:
      exists("app/sitemap.ts") && exists("app/opengraph-image.tsx") && exists("scripts/seo-check.mjs")
        ? "ok"
        : "missing",
    detail: "Sitemap, Open Graph et SEO gate présents.",
    group: "Qualité",
  });
  {
  ...
  try {
    const staging = JSON.parse(fs.readFileSync(path.join(root, "generated/staging-approval.json"), "utf8")) as {
      status?: string;
      url?: string;
    };
  ...
  checks.push({
    id: "staging",
    label: "Staging Vercel avant Production",
    status: stagingApproved ? "ok" : "warning",
    detail: stagingApproved
      ? "Preview testée et approuvée. Gate Production disponible."
      : "Obligatoire avant Production : staging:deploy → staging:test → staging:approve.",
    group: "Qualité",
  });
