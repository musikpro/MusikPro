...
const projectReady =
  fs.existsSync(path.join(root, "CLAUDE.md")) && fs.existsSync(path.join(root, ".claude/settings.json"));
...
if (verified) {
  console.log(`- Preuve: ${String(state.evidence || "test réel confirmé").slice(0, 180)}`);
...
console.log(
  "2. Utiliser son outil navigateur/computer use disponible, ou un MCP navigateur autorisé, pour ouvrir une page et vérifier visuellement son contenu.",
);
console.log("3. Après succès réel seulement:");
console.log(
  '   npm run computer-use:claude:mark -- --status=verified --evidence="page ouverte et vérifiée avec Claude Code"',
);
process.exitCode = 2;
