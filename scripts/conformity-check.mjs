...
const requiredFiles = [
  ...
  "docs/audit/general-refactor-v0.8.28.md",
  "lib/seo/metadata.ts",
  "app/robots.ts",
  "app/sitemap.ts",
  ...
  "tests/payments/capabilities.test.ts",
  ...
  "docs/architecture/starter-capabilities.md",
  ...
  "scripts/claude-code-prepare.mjs",
...
for (const rel of [
  ...
  try {
    ...
    fail(`JSON invalide: ${rel} — ${e.message}`);
  }
}
...
if (exists("package.json")) {
  ...
  const expectedScripts = [
    ...
    "security:db-check",
    "security:release",
    "security:release:online",
    "security:check",
    "security:audit",
    "mobile:check",
    "mobile:app:configure",
    "mobile:app:install",
    "mobile:app:prepare",
    "mobile:app:check",
    "ui:icons-check",
    ...
    "deploy:check",
    "doctor:production",
    "doctor:production:online",
    "doctor:kit",
    "dependencies:check",
    "dependencies:check:production",
    "kit:integrity",
    "kit:audit",
    "kit:verify",
    "kit:clean",
    ...
    "computer-use:claude:mark",
    "banani:prepare",
    "banani:check",
    "import-banani:check",
    "import-banani:analyze",
    "clients:crud:setup",
    "clients:crud:generate",
    "clients:crud:migrate",
    "clients:crud:check",
  ];
  for (const name of expectedScripts) scripts[name] ? ok(`Script npm: ${name}`) : fail(`Script npm manquant: ${name}`);
  const latest = Object.entries({ ...(pkg.dependencies || {}), ...(pkg.devDependencies || {}) }).filter(
    ([, v]) => v === "latest",
  );
  if (latest.length) fail(`Dépendances non figées: ${latest.map(([k]) => k).join(", ")}`);
  else ok("Dépendances principales figées");
}
...
for (const sensitive of [".env.local", ".env.production", ".env.development", ".env.test"]) {
  ...
  try {
    ...
    const tracked = execFileSync("git", ["ls-files", "--error-unmatch", sensitive], {
      cwd: root,
...
if (staleAudit.length) fail(`Anciens audits versionnés à la racine: ${staleAudit.join(", ")}`);
else ok("Audit racine consolidé");
if (staleChangelog.length) fail(`Anciens changelogs versionnés à la racine: ${staleChangelog.join(", ")}`);
...
if (exists(".gitignore")) {
  ...
  lines.has(".codex/config.toml")
    ? ok("Banani MCP config ignoré par Git")
    : fail(".codex/config.toml doit être ignoré par Git");
}
...
if (exists("lib/auth/index.ts")) {
  ...
  txt.includes("AUTH_EMAIL_PASSWORD_ENABLED") && txt.includes("AUTH_REQUIRE_EMAIL_VERIFICATION")
    ? ok("Auth email/password explicitement configurable")
    : fail("Auth email/password gating incomplet");
}
if (exists("lib/email/index.ts")) {
  const txt = read("lib/email/index.ts");
  txt.includes('NODE_ENV === "production"')
    ? ok("Email critique fail-closed en production")
    : fail("Email critique peut être silencieusement perdu en production");
}
if (exists("scripts/setup.mjs")) {
  const txt = read("scripts/setup.mjs");
  txt.includes("emailPasswordEnabled") && txt.includes("googleAuth")
    ? ok("Setup empêche un SaaS sans méthode d’auth")
    : fail("Setup ne valide pas la disponibilité d’une méthode d’auth");
}

// Security invariants
if (exists("lib/billing/webhook.ts")) {
  const txt = read("lib/billing/webhook.ts");
  txt.includes("storedEventSummary")
    ? ok("Webhooks minimisent le payload persisté")
    : fail("Webhooks: résumé minimisé manquant");
}
if (exists("lib/payments/routing.ts")) {
  const txt = read("lib/payments/routing.ts");
  txt.includes("providerRuntimeAllowed")
    ? ok("Filtrage runtime providers")
    : fail("Filtrage runtime providers manquant");
}
if (exists("app/api/payments/checkout/route.ts")) {
  const txt = read("app/api/payments/checkout/route.ts");
  txt.includes("isSafeProviderFallbackError")
    ? ok("Fallback paiement prudent")
    : fail("Fallback paiement ambigu non protégé");
}
if (exists("scripts/runtime-check.mjs")) ok("Runtime Node.js gate présent");
else fail("Runtime Node.js gate manquant");
if (exists("config/features.json") && exists("scripts/feature-inventory.mjs")) ok("Feature ownership manifest présent");
else fail("Feature ownership manifest manquant");
if (exists("lib/storage/cloudinary.ts")) {
  const txt = read("lib/storage/cloudinary.ts");
  txt.includes("detectImageType")
    ? ok("Cloudinary valide le contenu binaire")
    : fail("Cloudinary magic-byte validation manquante");
}
if (exists("components/ui/skeleton.tsx")) ok("Skeleton loaders présents");
if (exists("scripts/premium-icon-check.mjs") && exists("components/ui/premium-icon.tsx"))
  ok("Premium Icon Gate présent");
else fail("Premium Icon Gate incomplet");
if (
  exists("scripts/security-baseline-check.mjs") &&
  exists("config/security-routes.json") &&
  exists("config/security-rls.json") &&
  exists("db/security/rls-baseline.sql")
)
  ok("Security Baseline Gate présent");
else fail("Security Baseline Gate incomplet");
if (
  exists("scripts/general-refactor-check.mjs") &&
  exists("lib/security/request-guards.ts") &&
  exists("lib/payments/public-result.ts")
)
  ok("General Refactor Gate présent");
else fail("General Refactor Gate incomplet");
...
exists("package-lock.json")
  ? ok("package-lock.json présent")
  : warn(
      "package-lock.json absent dans le starter — lancez npm install avant la production; dependencies:check:production l’exigera.",
    );

// Generated config: optional before setup but warning at final conformity
if (!exists("africa-saas.config.json"))
  warn("africa-saas.config.json absent — normal seulement si le setup n’a pas encore été exécuté");
if (!exists("generated/deployment-handoff.md"))
  warn("Deployment handoff non généré — lancer npm run deploy:handoff avant mise en ligne");
if (!exists("generated/production-doctor.json") && !exists("generated/production-doctor.md"))
  warn("Production Doctor non généré — lancer npm run doctor:production avant mise en ligne");
...
fs.writeFileSync(
  path.join(generatedDir, "conformity-report.json"),
  JSON.stringify(
    { version: kitVersion, generatedAt: new Date().toISOString(), status, passes, warnings, failures },
    null,
    2,
  ),
);
