...
try {
  ...
  failures.push("package.json invalide");
}
...
for (const [name, version] of Object.entries(deps)) {
  if (typeof version !== "string" || /^(latest|next|\*|workspace:\*)$/i.test(version))
    failures.push(`Dépendance non figée: ${name}@${version}`);
}

console.log(
  `Dependency install check: ${failures.length ? "FAIL" : "PASS"}${production ? " (production)" : " (starter)"}`,
);
for (const n of notes) console.log(`✓ ${n}`);
for (const w of warnings) console.warn(`⚠ ${w}`);
for (const f of failures) console.error(`✗ ${f}`);
if (!production && warnings.length)
  console.log("Starter intact: ces avertissements sont attendus avant la première installation locale.");
