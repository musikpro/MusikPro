...
const cloudinaryEnabled =
  appCfg.cloudinaryEnabled === true ||
  ["CLOUDINARY_CLOUD_NAME", "CLOUDINARY_API_KEY", "CLOUDINARY_API_SECRET"].some(configured);
const upstashEnabled =
  appCfg.upstashEnabled === true || ["UPSTASH_REDIS_REST_URL", "UPSTASH_REDIS_REST_TOKEN"].some(configured);
...
for (const [id, p] of Object.entries(providers)) {
  ...
  providerRows.push({
    id,
    label: p.label || id,
    readiness: p.readiness || "unknown",
    enabled,
    env: names.map((name) => ({ name, configured: configured(name) })),
    webhook: `${base}/api/webhooks/${id}`,
  });
}

const visibleGroups = deployment.groups.filter(
  (group) => (paymentsEnabled || group.id !== "payments-core") && (cloudinaryEnabled || group.id !== "cloudinary"),
);
const variableRows = visibleGroups.flatMap((group) =>
  group.variables
    .filter(
      (v) =>
        (paymentsEnabled ||
          !["PAYMENT_WEBHOOK_BASE_URL", "PAYMENT_DEFAULT_PROVIDER", "CRON_SECRET"].includes(v.name)) &&
        (upstashEnabled || !["UPSTASH_REDIS_REST_URL", "UPSTASH_REDIS_REST_TOKEN"].includes(v.name)),
    )
    .map((v) => ({ ...v, group: group.label, configured: configured(v.name) })),
);
...
for (const group of visibleGroups) {
  ...
  md.push("|---|---|---:|---|---|---|");
  for (const v of group.variables.filter(
    (v) =>
      (paymentsEnabled || !["PAYMENT_WEBHOOK_BASE_URL", "PAYMENT_DEFAULT_PROVIDER", "CRON_SECRET"].includes(v.name)) &&
      (upstashEnabled || !["UPSTASH_REDIS_REST_URL", "UPSTASH_REDIS_REST_TOKEN"].includes(v.name)),
  )) {
    md.push(
      `| \`${v.name}\` | ${configured(v.name) ? "✅ CONFIGURÉE" : "❌ MANQUANTE"} | ${v.sensitive ? "Oui" : "Non"} | ${(v.vercel || []).join(" + ")} | ${v.required} | ${v.source} |`,
    );
  }
  md.push("");
}
...
for (const p of providerRows) {
  ...
  for (const e of p.env)
    md.push(
      `- ${e.configured ? "✅" : "❌"} \`${e.name}\` — ${e.configured ? "configurée localement" : "à renseigner"}`,
    );
...
md.push(
  cloudinaryEnabled
    ? "Cloudinary est activé : ajouter ses variables dans Vercel et effectuer un upload réel en staging."
...
md.push(
  "3. **Gate Staging obligatoire** — créer une Vercel Preview, tester `staging:test`, puis approuver avec `staging:approve`.",
);
md.push(
  "4. **Gate Production** — `deploy:production:check` doit passer pour le commit courant avant tout déploiement final.",
);
...
md.push(
  paymentsEnabled
    ? "9. **Gate Payments** — URLs webhook du domaine final + clés production seulement après sandbox validé."
    : "7. **Gate Payments** — ignoré : aucun paiement activé pour ce SaaS.",
);
md.push(
  paymentsEnabled
    ? "10. **Gate Cron** — exécuter `npm run cron:generate`, configurer `CRON_SECRET`, puis valider la réconciliation sur Vercel."
...
fs.writeFileSync(
  path.join(root, "generated/deployment-handoff.json"),
  JSON.stringify(
    {
      ...
      externalUrls: {
        googleOAuth: `${base}/api/auth/callback/google`,
        cron: paymentsEnabled ? `${base}/api/cron/reconcile-payments` : null,
