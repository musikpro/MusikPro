...
if (!fs.existsSync(designMd)) { console.error("✗ DESIGN.md manquant"); failed = true; }
else {
  const text = fs.readFileSync(designMd, "utf8");
  console.log(text.length > 200 ? "✓ DESIGN.md présent" : "⚠ DESIGN.md très court");
}

if (!fs.existsSync(bananiDir)) { console.error("✗ design/banani/screens manquant"); failed = true; }
