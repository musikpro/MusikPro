...
for (const group of deployment.groups || []) {
  for (const variable of group.variables || []) {
    ...
    if (variable.sensitive && variable.name.startsWith("NEXT_PUBLIC_"))
      errors.push(`Secret marqué public dans le registre: ${variable.name}`);
  }
}
...
for (const [key, value] of entries) {
  ...
  if (/^(?:sk_live_|ghp_|AIza|AKIA)/.test(value))
    errors.push(`Valeur ressemblant à un secret réel dans .env.example: ${key}`);
  if (
