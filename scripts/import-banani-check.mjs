...
catch (e) { console.error(`✗ imported-design.json invalide: ${e.message}`); process.exit(1); }
if (!Array.isArray(data.screens) || !data.screens.length) {
  console.error("✗ imported-design.json ne contient aucun écran."); process.exit(1);
}
const raw = fs.readFileSync(file, "utf8");
if (/Bearer\s+[A-Za-z0-9._-]{12,}|bnni_[A-Za-z0-9_-]+/i.test(raw)) {
  console.error("✗ Secret/token Banani détecté dans imported-design.json. Retire-le et régénère le token si nécessaire."); process.exit(1);
}
