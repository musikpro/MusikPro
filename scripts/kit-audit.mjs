...
for (const file of scriptFiles) {
  ...
  if (r.status !== 0) {
    syntaxFailure = `${path.relative(root, file)}\n${r.stderr || r.stdout}`;
    break;
  }
}
results.push({
  id: "script-syntax",
  label: `Syntaxe Node (${scriptFiles.length} scripts)`,
  status: syntaxFailure ? "fail" : "pass",
  exitCode: syntaxFailure ? 1 : 0,
  output: syntaxFailure || "Tous les scripts Node se parsèment correctement.",
});
...
results.push({
  id: "shell-syntax",
  label: `Syntaxe Shell (${shellFiles.length} scripts)`,
  status: shellFailures.length ? "fail" : "pass",
  exitCode: shellFailures.length ? 1 : 0,
  output: shellFailures.join("\n") || "Tous les scripts shell se parsèment correctement.",
});
...
results.push({
  id: "merge-markers",
  label: "Marqueurs de conflit Git",
  status: conflictFiles.length ? "fail" : "pass",
  exitCode: conflictFiles.length ? 1 : 0,
  output: conflictFiles.join("\n") || "Aucun marqueur de conflit Git non résolu.",
});
...
for (const file of jsonFiles) {
  try {
    ...
    invalidJson.push(`${path.relative(root, file)}: ${e.message}`);
  }
}
results.push({
  id: "json",
  label: `JSON valides (${jsonFiles.length} fichiers)`,
  status: invalidJson.length ? "fail" : "pass",
  exitCode: invalidJson.length ? 1 : 0,
  output: invalidJson.join("\n") || "Tous les JSON sont valides.",
});
...
const sourceFiles = walk(root).filter(
  (f) => /\.(?:ts|tsx|js|jsx|mjs|cjs)$/.test(f) && !f.includes(`${path.sep}generated${path.sep}`),
);
...
results.push({
  id: "local-imports",
  label: `Imports locaux (${sourceFiles.length} fichiers source)`,
  status: missingImports.length ? "fail" : "pass",
  exitCode: missingImports.length ? 1 : 0,
...
const firstRunOk =
  fs.existsSync(firstRunFile) &&
  firstRunPkg.scripts?.["first-run"] === "node scripts/first-run.mjs" &&
  firstRunPkg.scripts?.["first-run:install"] === "node scripts/first-run.mjs --install";
results.push({
  id: "first-run",
  label: "Premier démarrage guidé",
  status: firstRunOk ? "pass" : "fail",
  exitCode: firstRunOk ? 0 : 1,
  output: firstRunOk
    ? "first-run et first-run:install sont câblés."
    : "Commande de premier démarrage absente ou incohérente.",
});
...
results.push({
  id: "script-targets",
  label: "Cibles des scripts npm",
  status: missingTargets.length ? "fail" : "pass",
  exitCode: missingTargets.length ? 1 : 0,
  output: missingTargets.join("\n") || "Aucune cible locale manquante.",
});
console.log(`${missingTargets.length ? "✗" : "✓"} Cibles des scripts npm`);

const warnings = [];
if (!fs.existsSync(path.join(root, "package-lock.json")))
  warnings.push("package-lock.json absent : lancez npm install avant certification production.");
if (!fs.existsSync(path.join(root, "node_modules")))
  warnings.push(
    "node_modules absent : lint/typecheck/tests/build/npm audit dynamiques non exécutés par cet audit statique.",
  );
