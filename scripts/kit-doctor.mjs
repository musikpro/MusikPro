...
let config = null;
try { config = JSON.parse(fs.readFileSync(path.join(root, "africa-saas.config.json"), "utf8")); } catch {}
const mobileEnabled = Boolean(config?.mobileApp?.enabled || config?.mobile?.appEnabled || config?.mobileAppEnabled);
if (mobileEnabled) {
  add("Mobile: URL de production", /^https:\/\//i.test(config?.mobileApp?.productionUrl || config?.mobile?.productionUrl || ""), "HTTPS requis pour le mode WebView");
  if (process.platform === "darwin") {
    try { execFileSync("xcodebuild", ["-version"], { stdio: "ignore" }); add("Mobile iOS: Xcode", true); } catch { add("Mobile iOS: Xcode", false, "à installer avant le build iPhone"); }
  }
  try { execFileSync("java", ["-version"], { stdio: "ignore" }); add("Mobile Android: JDK", true); } catch { add("Mobile Android: JDK", false, "à installer avant le build Android"); }
}
...
console.log("Commandes utiles: npm run setup-saas · npm run kit:verify · npm run kit:integrity · npm run dependencies:check · npm run verify:code");
