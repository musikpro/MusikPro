...
const requiredFiles = [
  ...
  "components/readiness-check-card.tsx",
  "config/readiness-ui.json",
...
for (const rel of refactorInstructionFiles) {
  ...
  if (
    ...
    !/(non régressive|préserver les fonctionnalités existantes)/i.test(content)
  ) {
...
if (fs.existsSync(productionStatePagePath)) {
  ...
  if (!/href[:=]\s*["']\/admin\/production-doctor["']/.test(adminNavText) || !/État production/.test(adminNavText)) {
    failures.push(
      "app/admin/layout.tsx (ou components/admin/AdminShell.tsx) — menu propriétaire « État production » absent",
    );
  }
  if (
    !/État production/.test(productionStatePage) ||
    !/rapport CLI reste la source de vérité/i.test(productionStatePage)
  ) {
...
if (fs.existsSync(dashboardPath)) {
  ...
  if (!/npm run kit:verify/.test(dashboard))
    failures.push("components/setup-saas-dashboard.tsx — kit:verify command missing from dashboard");
  if (!/npm run kit:full-test/.test(dashboard))
    failures.push("components/setup-saas-dashboard.tsx — kit:full-test command missing from dashboard");
  if (!/npm run first-run/.test(dashboard))
    failures.push("components/setup-saas-dashboard.tsx — first-run command missing from dashboard");
  if (!/Staging Vercel — obligatoire avant Production/.test(dashboard) || !/staging:approve/.test(dashboard))
    failures.push("components/setup-saas-dashboard.tsx — mandatory staging section missing");
  if (!/Agents IA & Computer Use/.test(dashboard) || !/computer-use:claude:check/.test(dashboard))
    failures.push("components/setup-saas-dashboard.tsx — Claude Code / Computer Use readiness section missing");
}
