...
async function getNgrokUrl() {
  try {
    ...
    const tunnel =
      (data.tunnels || []).find((t) => String(t.public_url || "").startsWith("https://")) || (data.tunnels || [])[0];
...
if (!enabled.length) {
  ...
  fs.writeFileSync(
    path.join(root, "generated/local-payment-lab.md"),
    "# Local Payment Lab\n\nPaiements désactivés : aucun provider n’est configuré. Ce test est optionnel pour ce SaaS.\n",
  );
  fs.writeFileSync(
    path.join(root, "generated/local-payment-lab.json"),
    JSON.stringify({ generatedAt: new Date().toISOString(), paymentsEnabled: false, enabledProviders: [] }, null, 2),
  );
...
const lines = [
  ...
  "",
  ...(publicUrl
    ? enabled.map((p) => `- ${p}: \`${webhookUrls[p]}\``)
    : ["Démarre d’abord `ngrok http " + port + "`, puis relance `npm run payments:local`."]),
  "",
  "## Règle de sécurité",
  "",
...
if (publicUrl)
  console.log(
    `Env webhook: PAYMENT_WEBHOOK_BASE_URL=${publicUrl}${args.has("--write-env") ? " (écrit dans .env.local)" : ""}`,
  );
if (!ngrokInstalled) {
  console.log("\nAction: installe ngrok puis configure ton authtoken.");
  process.exitCode = 1;
} else if (!appRunning || !publicUrl) {
  console.log(`\nAction: terminal A -> npm run dev ; terminal B -> ngrok http ${port}`);
  process.exitCode = 1;
