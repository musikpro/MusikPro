...
const args = Object.fromEntries(process.argv.slice(2).filter(a=>a.startsWith('--')).map(a=>{const [k,...r]=a.slice(2).split('=');return [k,r.length?r.join('='):true];}));
...
const productionUrl = String(args.url || config.mobileApp?.productionUrl || (String(config.appUrl||'').startsWith('https://') ? config.appUrl : '')).trim();
const platforms = String(args.platforms || (config.mobileApp?.platforms || ['android','ios']).join(','))
  .split(',').map(x=>x.trim().toLowerCase()).filter(Boolean);
if (!/^[a-zA-Z][a-zA-Z0-9]*(\.[a-zA-Z][a-zA-Z0-9_-]*){1,}$/.test(appId)) throw new Error('Provide --app-id=com.example.app (reverse-domain identifier).');
...
if (!platforms.length || platforms.some(p=>!['android','ios'].includes(p))) throw new Error('Platforms must contain android and/or ios.');
