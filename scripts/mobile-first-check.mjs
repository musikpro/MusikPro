...
const planner = read("scripts/generate-implementation-plan.mjs");
function add(id, ok, detail) {
  checks.push({ id, status: ok ? "PASS" : "FAIL", detail });
}
add(
  "mobile-css",
  /mobile-bottom-nav/.test(css) && /@media\s*\(min-width:\s*768px\)/.test(css),
  "Navigation mobile dédiée + breakpoint desktop",
);
add("safe-area", /safe-area-inset-bottom/.test(css), "Gestion de la safe area iPhone/Android");
add(
  "touch-target",
  /min-height:\s*44px/.test(css) || /min-height:\s*48px/.test(css),
...
if (failed.length) {
  console.error(`\n${failed.length} contrôle(s) mobile FAIL.`);
  process.exit(1);
