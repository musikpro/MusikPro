...
for (const file of files) {
  ...
  if (/export\s+const\s+runtime\s*=\s*['"]edge['"]/.test(src)) errors.push(`${rel}: runtime=edge interdit pour ce starter`);
  if (!/export\s+const\s+runtime\s*=\s*['"]nodejs['"]/.test(src)) errors.push(`${rel}: export const runtime = "nodejs" manquant`);
}
