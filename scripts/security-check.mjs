...
for (const f of [".env", ".env.local", ".env.production", ".env.development", ".env.test"]) {
  if (fs.existsSync(f)) {
    ...
    if (/NEXT_PUBLIC_.*(?:SECRET|PRIVATE|DATABASE|TOKEN|API_KEY)/i.test(text))
      errors.push(`${f}: possible secret exposed via NEXT_PUBLIC_*`);
    if (/BETTER_AUTH_SECRET\s*=\s*(?:changeme|secret|123|)$/im.test(text))
      errors.push(`${f}: BETTER_AUTH_SECRET is weak or empty`);
  }
}
for (const f of [
  "SECURITY.md",
  ...
  "lib/payments/capabilities.ts",
  "lib/payments/providers/paydunya.ts",
  ...
  "lib/payments/providers/moneroo.ts",
  "lib/payments/providers/paytech.ts",
  "lib/payments/providers/bictorys.ts",
  ...
  "lib/doctor/read-report.ts",
  "app/admin/production-doctor/page.tsx",
  "docs/production-doctor.md",
  "db/schema/index.ts",
  "lib/security/rate-limit.ts",
  "scripts/local-payment-lab.mjs",
  "docs/payments/local-payment-lab.md",
  "scripts/mobile-first-check.mjs",
  "docs/mobile/mobile-first-delivery.md",
  "components/mobile-bottom-nav.tsx",
  "lib/payments/webhook-url.ts",
  "components/ui/skeleton.tsx",
  "app/loading.tsx",
  "app/dashboard/loading.tsx",
  "app/admin/loading.tsx",
  "docs/ui/skeleton-loading.md",
  "scripts/loading-check.mjs",
  "lib/seo/site.ts",
  "lib/seo/metadata.ts",
  ...
  "docs/cloudflare/domain-dns.md",
  "scripts/setup-cloudinary.mjs",
  "docs/storage/cloudinary.md",
  ...
  "vitest.config.ts",
  "eslint.config.mjs",
  "tests/payments/provider-base.test.ts",
  "tests/config/provider-environment.test.ts",
  "lib/payments/configured.ts",
  ...
  "lib/api/client.ts",
  "lib/cron/auth.ts",
  "scripts/smoke-system.mjs",
  "scripts/generate-vercel-cron.mjs",
  ...
  "scripts/security-db-check.mjs",
  "config/security-routes.json",
  "config/security-rls.json",
  "db/security/rls-baseline.sql",
  "docs/security/security-baseline-gate.md",
  ".github/workflows/security-guard.yml",
  "scripts/general-refactor-check.mjs",
  "lib/security/request-guards.ts",
  "lib/payments/public-result.ts",
  "tests/security/public-checkout-result.test.ts",
  "tests/security/request-guards.test.ts",
  "scripts/dependency-security-floor.mjs",
...
for (const requiredIgnore of [".env.production", ".env.development", ".env.test", ".codex/config.toml"]) {
  if (!gitignoreText.split(/\r?\n/).includes(requiredIgnore))
    errors.push(`.gitignore must explicitly ignore ${requiredIgnore}`);
}
const paydunyaText = fs.readFileSync("lib/payments/providers/paydunya.ts", "utf8");
if (!/money:\s*Number\.isFinite\(normalized\.amount\)/.test(paydunyaText))
  errors.push("PayDunya verifyPayment must return normalized money for reconciliation");
const providerBaseText = fs.readFileSync("lib/payments/provider-base.ts", "utf8");
if (providerBaseText.includes("JSON.stringify(body)"))
  errors.push("Provider HTTP errors must not persist raw provider bodies");
const webhookText = fs.readFileSync("lib/billing/webhook.ts", "utf8");
if (!webhookText.includes("storedEventSummary"))
  errors.push("Webhook persistence must minimize raw provider payload/PII");
...
if (!configuredText.includes("providerEnvironmentConfigured"))
  errors.push("Optional provider environment guard missing");
const routingText = fs.readFileSync("lib/payments/routing.ts", "utf8");
if (
  !routingText.includes("providerRuntimeAllowed(p)") ||
  !routingText.includes("providerEnvironmentConfigured(p)") ||
  !routingText.includes("if (!cc)")
)
  errors.push("Provider readiness/runtime/environment filtering must apply even when no country is resolved");
const paymentHealthText = fs.readFileSync("lib/payments/health.ts", "utf8");
if (!paymentHealthText.includes("minimumAttempts = 5") || !paymentHealthText.includes("successThreshold = 0.2"))
  errors.push("DB-backed provider degradation guard missing or altered");
const checkoutText = fs.readFileSync("app/api/payments/checkout/route.ts", "utf8");
if (
  !checkoutText.includes("isSafeProviderFallbackError") ||
  !checkoutText.includes('status: safeFallback ? "failed" : "uncertain"')
)
  errors.push("Checkout router must stop automatic fallback on ambiguous provider state");
if (!checkoutText.includes("!x.degraded"))
  errors.push("Checkout must exclude recently degraded providers from automatic selection");
if (!checkoutText.includes("publicCheckoutResult(result)"))
  errors.push("Checkout must sanitize provider responses before returning them to browsers");
if (
  !checkoutText.includes("rejectCrossSiteMutation") ||
  !checkoutText.includes("rejectOversizedRequest") ||
  !checkoutText.includes("requireContentType")
)
  errors.push("Checkout request-level origin/body/content-type guards missing");
const dashboardLayoutSecurity = fs
  .readFileSync("app/dashboard/layout.tsx", "utf8")
  .replace(/\/\*[\s\S]*?\*\//g, "")
  .replace(/(^|[^:])\/\/.*$/gm, "$1");
...
if (!fs.existsSync("package-lock.json"))
  warnings.push("package-lock.json absent: run npm install once and commit the generated lockfile before production.");
...
if (pkgJson.devDependencies?.auth !== pkgJson.dependencies?.["better-auth"])
  warnings.push(
    "Better Auth CLI and better-auth versions differ; review compatibility before auth schema regeneration.",
  );
if (!String(pkgJson.scripts?.["auth:generate"] || "").includes("--config ./lib/auth/index.ts"))
  errors.push("auth:generate must point explicitly to lib/auth/index.ts");
...
if (!schemaText.includes("subscriptions_user_plan_unique"))
  errors.push("Missing subscription user/plan unique constraint");
const reconcileText = fs.readFileSync("lib/billing/reconcile.ts", "utf8");
if (!reconcileText.includes("lastPaymentId") || !reconcileText.includes("setWhere"))
  errors.push("Payment fulfilment idempotency guard missing");
if (!reconcileText.includes("sameMoney("))
  errors.push("Payment reconciliation must compare normalized monetary units rather than raw floating-point equality");

const chariowText = fs.readFileSync("lib/payments/providers/chariow.ts", "utf8");
if (
  !chariowText.includes('searchParams.get("secret")') ||
  !chariowText.includes("safeEqual(received, webhookSecret)") ||
  !chariowText.includes("x-pulse-delivery-id")
)
  errors.push("Chariow Pulse URL-secret verification/idempotency handling missing");
if (!schemaText.includes('numeric("amount"') || !schemaText.includes('numeric("provider_amount"'))
  errors.push("Money columns must support decimal currencies");
const webhookUrlText = fs.readFileSync("lib/payments/webhook-url.ts", "utf8");
if (!webhookUrlText.includes("PAYMENT_WEBHOOK_BASE_URL"))
  errors.push("Dedicated local webhook base URL support missing");
const rootLayoutText = fs.readFileSync("app/layout.tsx", "utf8");
if (!rootLayoutText.includes("<body suppressHydrationWarning>"))
  errors.push(
    "Root body hydration guard missing; browser extensions can inject body attributes and trigger noisy hydration overlays",
  );

const mobileCss = fs.readFileSync("app/globals.css", "utf8");
if (!mobileCss.includes("mobile-bottom-nav") || !mobileCss.includes("safe-area-inset-bottom"))
  errors.push("Mobile-first app shell/safe-area support missing");

const skeletonText = fs.readFileSync("components/ui/skeleton.tsx", "utf8");
if (!skeletonText.includes("SkeletonTable") || !skeletonText.includes("DashboardSkeleton"))
  errors.push("Reusable skeleton primitives missing");
if (!mobileCss.includes("prefers-reduced-motion") || !mobileCss.includes("skeleton-shimmer"))
  errors.push("Accessible skeleton shimmer/reduced-motion support missing");

const seoMetaText = fs.readFileSync("lib/seo/metadata.ts", "utf8");
if (
  !seoMetaText.includes("canonical") ||
  !seoMetaText.includes("openGraph") ||
  !seoMetaText.includes("summary_large_image")
)
  errors.push("SEO canonical/Open Graph/Twitter metadata support missing");
const robotsText = fs.readFileSync("app/robots.ts", "utf8");
if (!robotsText.includes("/dashboard/") || !robotsText.includes("/admin/") || !robotsText.includes("/api/"))
  errors.push("Private routes must remain excluded from crawling");
...
if (
  setupText.includes("Payment providers, comma-separated") ||
  setupText.includes("Choose at least one payment provider")
)
  errors.push("Core setup must not require payment providers");
const validateText = fs.readFileSync("scripts/validate-config.mjs", "utf8");
if (validateText.includes("At least one provider is required"))
  errors.push("Configuration validation must allow SaaS without payments");
const setupSaasText = fs.readFileSync("scripts/setup-saas.mjs", "utf8");
if (!setupSaasText.includes("Phase 17") && !setupSaasText.includes("paiements (OPTIONNEL)"))
  warnings.push("Review optional payment phase wording");
const cloudflareSetup = fs.readFileSync("scripts/setup-cloudflare.mjs", "utf8");
if (!cloudflareSetup.includes("--none") || !cloudflareSetup.includes("phase=18"))
...
if (
  ...
  !workflowText.includes("Cloudinary") ||
  !workflowText.includes("conformity:check")
)
  errors.push("/setup-saas roadmap must include 21 phases, optional services and final conformity check");
const cloudinaryText = fs.readFileSync("lib/storage/cloudinary.ts", "utf8");
if (!cloudinaryText.includes("MAX_IMAGE_BYTES") || !cloudinaryText.includes("image/svg+xml") === false) {
  /* SVG is intentionally absent from allowed set */
}
...
if (!readyRouteText.includes("503") || !readyRouteText.includes("getReadinessReport"))
  errors.push("Readiness endpoint must return 503 when dependencies are unavailable");
if (!pkgJson.scripts?.test || !pkgJson.scripts?.lint || !pkgJson.scripts?.["audit:prod"])
  errors.push("Test/lint/audit scripts are required");
if (
  !pkgJson.scripts?.["security:baseline"] ||
  !pkgJson.scripts?.["security:db-check"] ||
  !pkgJson.scripts?.["security:release"]
)
  errors.push("Continuous security baseline/db/release gates are required");
if (!pkgJson.devDependencies?.vitest || !pkgJson.devDependencies?.eslint || !pkgJson.devDependencies?.prettier)
  errors.push("Vitest/ESLint/Prettier dev dependencies are required");
...
if (
  !cloudinaryAdapterText.includes("detectImageType") ||
  (!cloudinaryAdapterText.includes("magic") && !cloudinaryAdapterText.includes("Image content does not match"))
)
  errors.push("Cloudinary upload must validate magic bytes, not only client MIME");
const apiClientText = fs.readFileSync("lib/api/client.ts", "utf8");
if (!apiClientText.includes('method === "GET"') || !apiClientText.includes('method === "HEAD"'))
  errors.push("API client retry policy must be restricted to GET/HEAD");
const cronAuthText = fs.readFileSync("lib/cron/auth.ts", "utf8");
if (!cronAuthText.includes("timingSafeEqual") || !cronAuthText.includes("NODE_ENV"))
  errors.push("Central cron auth helper missing fail-safe comparison");

const authText = fs.readFileSync("lib/auth/index.ts", "utf8");
if (!authText.includes("AUTH_EMAIL_PASSWORD_ENABLED") || !authText.includes("AUTH_REQUIRE_EMAIL_VERIFICATION"))
  errors.push("Auth must explicitly gate email/password and email verification by env");
if (!emailText.includes('NODE_ENV === "production"') || !emailText.includes("Resend"))
  errors.push("Critical auth email delivery must fail closed in production when Resend is missing");
if (!setupText.includes("emailPasswordEnabled") || !setupText.includes("googleAuth"))
      ...
      try {
        ...
        if (
          trace.files.some(
            (value) =>
              typeof value === "string" &&
              /(?:^|\/)(?:\.codex|\.agents|\.git)(?:\/|$)|(?:^|\/)\.env(?:\.|$)/.test(value.replaceAll("\\", "/")),
          )
        ) {
