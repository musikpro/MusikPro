...
try {
  ...
  const recommended = (preset.recommendedProviders || []).filter(p => providersCatalog[p] && !["scaffold","merchant-validation"].includes(providersCatalog[p].readiness));
  ...
  for (const p of selected) {
    ...
    if (["scaffold","merchant-validation"].includes(providersCatalog[p].readiness)) throw new Error(`${p} cannot be enabled (${providersCatalog[p].readiness})`);
  }
