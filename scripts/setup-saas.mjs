...
const bananiMcpConfigured = /\[\s*mcp_servers\.banani\s*\]/i.test(codexConfigText) && /^\s*url\s*=\s*["'][^"']+["']/mi.test(codexConfigText) && /Authorization/i.test(codexConfigText) && /Bearer\s+[^"'\s}]+/i.test(codexConfigText);
...
try { importedDesign = fs.existsSync(importedDesignPath) ? JSON.parse(fs.readFileSync(importedDesignPath, "utf8")) : null; } catch {}
const bananiImported = Boolean(importedDesign?.screens?.length) && Boolean(screens?.screens?.length) && !/Remplacer ou compléter cet exemple|Exemple de structure attendue/i.test(screensText);
...
const phases = [
    ...
    benefit: "Elle permet à l’IA de vérifier visuellement les pages, responsive, formulaires, OAuth, uploads, paiements sandbox, previews Vercel, SEO et domaine final au lieu de se limiter au code.",
    checks: [
      { label: "Skill /computer-use", ok: exists(".agents/skills/computer-use/SKILL.md"), detail: "Workflow navigateur du kit" },
      { label: "Guide Browser Tools", ok: exists("docs/computer-use/antigravity-browser.md"), detail: "Activation et règles de sécurité" },
      { label: "Browser Subagent vérifié", ok: computerUseVerified, detail: computerUseVerified ? `Preuve locale: ${computerUseState?.evidence || "test navigateur réussi"}` : "NON VÉRIFIÉ — un vrai test navigateur est requis" },
    ],
    actions: [
    ...
    validate: "La phase passe uniquement après une vraie action Browser Subagent observée. Hors Antigravity, utiliser un outil navigateur équivalent ou marquer cette phase `skipped` avec justification.",
  },
    {
    n: 3,
    ...
    checks: [
      ...
      { label: "Banani MCP via .codex/config.toml", ok: bananiMcpConfigured, detail: bananiMcpConfigured ? "Configuré localement (token non affiché)" : (codexConfigExists ? "Fichier présent mais Banani MCP non configuré" : "À créer avec npm run banani:prepare") },
      { label: "Écrans Banani réellement importés", ok: bananiImported, detail: bananiImported ? "Snapshot MCP + inventaire normalisé présents" : "Lancer /import-banani après connexion MCP" },
      { label: "Gap analysis Banani → Starter", ok: gapGenerated, detail: gapGenerated ? "generated/banani-gap-analysis.md présent" : "À générer avec npm run import-banani:analyze" },
    ],
    actions: [
    ...
    validate: "`npm run banani:check` et `npm run import-banani:check` passent, le gap analysis existe et le handoff design passe avant toute construction massive.",
  },
    {
    n: 10,
    ...
    benefit: 'Cette phase évite de coder au hasard et fournit Create/Read/Update/Delete Clients prêt à brancher, avec Zod, Better Auth, rate limiting et RLS.',
    checks: [
      ...
      { label: "CRUD Clients Prisma", ok: exists("prisma/schema.prisma") && exists("app/api/clients/route.ts") && exists("app/api/clients/[id]/route.ts"), detail: "Scaffold sécurisé disponible après Banani" },
      { label: "Guide CRUD Clients", ok: exists("docs/features/clients-crud.md"), detail: "Ordre post-Banani documenté" },
    ],
    actions: [
    ...
    validate: "Le plan existe, le CRUD Clients passe `npm run clients:crud:check`, et la migration n’est appliquée qu’après import Banani si la fonctionnalité est réellement utilisée.",
  },
    {
    n: 11,
  ...
  },
  {
    ...
    checks: [
      ...
      { label: "Staging Vercel approuvé", ok: exists("generated/staging-approval.json"), detail: exists("generated/staging-approval.json") ? "Rapport d’approbation présent" : "Production bloquée tant que staging:approve n’a pas réussi" },
    ],
    actions: [
    ...
    validate: "`npm run staging:approve` doit réussir et `npm run deploy:production:check` doit passer. Sans approbation staging du commit courant, la Production reste bloquée.",
  },
  {
    n: 16,
    ...
    benefit: "Cette phase peut réduire la charge et la latence sur les lectures fréquentes, centraliser le rate limiting entre instances Vercel et conserver des états temporaires avec TTL.",
    checks: [
      { label: "Décision Upstash", ok: config?.upstashEnabled === false || config?.upstashEnabled === true, required: true, detail: upstashOk ? "Redis REST configuré" : config?.upstashEnabled ? "Activé mais variables incomplètes" : "Optionnel — non activé" },
      { label: "Variables Redis", ok: !config?.upstashEnabled || upstashOk, required: true, detail: upstashOk ? "URL + token présents" : "UPSTASH_REDIS_REST_URL / TOKEN absents" },
    ...
    validate: "La phase peut être SKIPPED. Si activée, `npm run upstash:check:online` doit réussir avant production et `/api/readyz` doit signaler Redis OK.",
  },
  {
    n: 17,
  ...
  },
  {
    ...
    checks: [
      { label: "Production Doctor prêt", ok: doctorReady, detail: doctorReady ? "Verdict READY" : (doctorGenerated ? `Verdict ${doctorReport?.verdict || "inconnu"} — corriger avant production` : "À exécuter après staging/domaine") },
    ...
    validate: "`npm run deploy:production:check` passe, le Production Doctor est satisfaisant, les flux externes sont testés et `npm run conformity:check` retourne PASS sans anomalie bloquante.",
  }
  ,{
    n: 21,
    ...
    benefit: "Elle permet de publier rapidement le SaaS en Android/iPhone en réutilisant le site en ligne, tout en gardant la partie mobile complètement optionnelle et réversible.",
    checks: config?.mobileAppEnabled ? [
      { label: "Mobile App activée", ok: true, detail: (config.mobileApp?.platforms || []).join(" + ") || "plateformes non définies" },
      { label: "URL production HTTPS", ok: /^https:\/\//.test(String(config.mobileApp?.productionUrl || "")), detail: config.mobileApp?.productionUrl || "URL manquante" },
      { label: "Préparation Capacitor", ok: config.mobileApp?.prepared === true && exists("capacitor.config.ts"), detail: config.mobileApp?.prepared ? "Projet natif préparé" : "À préparer" },
    ] : [
    ...
    validate: config?.mobileAppEnabled ? "`npm run mobile:app:check` doit réussir et les builds natifs doivent être testés sur appareils réels avant soumission aux stores." : "SKIPPED implicitement : le pipeline mobile est optionnel et le SaaS Web reste complet sans lui.",
  }
];
...
if (next) {
  ...
  if (!computerUseVerified && next.n > 2) report += `> ⚠️ Computer Use n’est pas encore marqué VERIFIED. Revenir à la Phase 2 avant de considérer les validations visuelles comme fiables.\n\n`;
...
fs.writeFileSync(path.join(root, "generated/setup-saas-report.json"), JSON.stringify({ version: kitVersion, generatedAt: new Date().toISOString(), greenCount, total: phases.length, nextPhase: next?.n || null, phases }, null, 2));
console.log(report);
