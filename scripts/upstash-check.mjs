...
const cfg = (() => { try { return JSON.parse(fs.readFileSync(path.join(root,"africa-saas.config.json"),"utf8")); } catch { return null; } })();
const env = {};
...
if (cfg?.upstashEnabled === false && !url && !token) { console.log("Upstash check: SKIPPED (optional / disabled)"); process.exit(0); }
if (!url || !token) { console.error("Upstash check: FAIL — REST URL/token incomplete"); process.exit(1); }
if (!/^https:\/\//i.test(url)) { console.error("Upstash check: FAIL — REST URL must use HTTPS"); process.exit(1); }
console.log("Upstash config: PASS — credentials present (token hidden)");
if (!online) process.exit(0);
try {
  const response=await fetch(url.replace(/\/$/,""), {method:"POST",headers:{Authorization:`Bearer ${token}`,"Content-Type":"application/json"},body:JSON.stringify(["PING"]),signal:AbortSignal.timeout(5000)});
