...
if (c.storage === "cloudflare-r2") errors.push("Cloudflare R2 is not implemented yet; use storage=none until the storage adapter is added");
...
if (emailPasswordEnabled && c.email !== "resend") errors.push("Email/password auth requires email=resend in this starter");
...
if (c.mobileAppEnabled != null && typeof c.mobileAppEnabled !== "boolean") errors.push("mobileAppEnabled must be boolean when present");
if (c.mobileAppEnabled === true) {
  const m = c.mobileApp || {};
  if (!/^[a-zA-Z][a-zA-Z0-9]*(\.[a-zA-Z][a-zA-Z0-9_-]*){1,}$/.test(String(m.appId || ""))) errors.push("mobileApp.appId must be a reverse-domain id when mobile is enabled");
  if (!/^https:\/\//.test(String(m.productionUrl || ""))) errors.push("mobileApp.productionUrl must use HTTPS when mobile is enabled");
  if (!Array.isArray(m.platforms) || m.platforms.length === 0 || m.platforms.some((p) => !["android","ios"].includes(p))) errors.push("mobileApp.platforms must contain android and/or ios");
}
...
if (enabled.length > 0 && !c.defaultProvider) errors.push("defaultProvider is required when payment providers are enabled");
...
console.log(c.mobileAppEnabled ? `✓ Mobile app: enabled (${(c.mobileApp?.platforms || []).join(", ")})` : "✓ Mobile app: disabled / optional");
