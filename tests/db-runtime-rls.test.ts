...
describe.runIf(process.env.RUN_NEON_RLS_TEST === "1")(
  "Drizzle runtime RLS",
  () => {
    it("isolates authenticated queries and clears pooled transaction context", async () => {
      ...
      try {
        for (const id of ids) {
          await db
            .insert(user)
            .values({ id, name: "RLS test", email: `${id}@example.invalid` });
          await service.insert(credits).values({ id, userId: id, balance: 1 });
        }
        const query = () =>
          db.select().from(credits).where(inArray(credits.userId, ids));
        ...
        expect(await query()).toEqual([]);
        await expect(userQuery("", query())).rejects.toThrow(
          "Invalid authenticated database identity",
        );
      } finally {
