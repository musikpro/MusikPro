...
describe("Payment provider dependency and validation regressions", () => {
  ...
  const payload = '{"id":"evt_test","name":"transaction.approved","entity":{"id":42}}';
  const request = (body: string, timestamp = Math.floor(Date.now() / 1000), signedBody = body) => {
  ...
  it("rejects tampered and expired FedaPay signatures", async () => {
    vi.stubEnv("FEDAPAY_WEBHOOK_SECRET", secret);
    const provider = new FedapayProvider();
    expect(await provider.verifyWebhook(request(payload.replace("42", "43"), undefined, payload))).toBe(false);
    expect(await provider.verifyWebhook(request(payload, Math.floor(Date.now() / 1000) - 600))).toBe(false);
  });
  it("preserves the verified Moneroo amount, currency and provider metadata", async () => {
    ...
    expect(paymentSummarySchema.parse(result.raw).data?.metadata?.app_reference).toBe("local_1");
  });
